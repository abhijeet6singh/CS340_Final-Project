...
-- Step 4 Update:
-- This DDL file is now inside a stored procedure wrapper.
-- The website RESET button will call sp_reset_peak_apparel().
-- Calling this procedure drops all project tables, recreates them,
-- and inserts the original sample data.

DROP PROCEDURE IF EXISTS sp_reset_peak_apparel;

DELIMITER //

CREATE PROCEDURE sp_reset_peak_apparel()
BEGIN
...
-- This call creates/resets the database immediately when the file is imported.
CALL sp_reset_peak_apparel();